...
<template>
  <div>
    <header class="mb-5 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold text-stone-900">Restaurants</h1>
        <p class="text-sm text-stone-500">Every restaurant on the platform.</p>
      </div>
      <button
    ...
    <p v-if="loading && !restaurants.length" class="text-sm text-stone-400">Loading…</p>
    <p
          ...
          <tbody class="divide-y divide-stone-100">
            <tr v-for="r in restaurants" :key="r.id" :class="busyId === r.id && 'opacity-50'">
              <td class="px-4 py-3">
                ...
                <p class="text-xs text-stone-400">{{ r.city || '—' }}{{ r.country ? `, ${r.country}` : '' }}</p>
              </td>
              <td class="px-4 py-3">
                <template v-if="r.owner">
                  <p class="text-stone-700">{{ r.owner.full_name }}</p>
                  <p class="text-xs text-stone-400">{{ r.owner.email }}</p>
                </template>
                <span v-else class="text-stone-300">—</span>
              </td>
              <td class="px-4 py-3 text-center tabular-nums text-stone-600">{{ r.users_count }}</td>
              <td class="px-4 py-3 text-center tabular-nums text-stone-600">{{ r.orders_count }}</td>
              <td class="px-4 py-3">
                <select
                  :value="r.status"
              ...
              <td class="px-4 py-3">
                <div class="flex items-center justify-end gap-1">
                  <a
                    :href="`/r/${r.slug}`"
                    target="_blank"
                    class="rounded-lg p-1.5 text-stone-400 transition hover:bg-stone-100 hover:text-stone-600"
      ...
      <ul class="divide-y divide-stone-100 sm:hidden">
        <li v-for="r in restaurants" :key="r.id" class="p-4" :class="busyId === r.id && 'opacity-50'">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <p class="truncate font-semibold text-stone-800">{{ r.name }}</p>
              <p class="truncate text-xs text-stone-400">{{ r.owner?.email || 'No owner' }}</p>
